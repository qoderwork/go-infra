...
// that implements the io.Reader and io.Writer interfaces.
//
// Derived from gnet/pkg/buffer/ring (https://github.com/panjf2000/gnet),
// licensed under MIT.
package ring
...
import (
	...
	"github.com/qoderwork/go-infra/bytepool"
	"github.com/qoderwork/go-infra/bytesconv"
...
func New(size int) *Buffer {
	...
	size = mathutil.CeilToPowerOfTwo(size)
	return &Buffer{
		buf:     make([]byte, size),
		size:    size,
		isEmpty: true,
...
// The returned slices may point to two separate regions (head and tail)
// when the data wraps around the end of the buffer.
...
func (rb *Buffer) WriteString(s string) (int, error) {
	return rb.Write(bytesconv.StringToBytes(s))
}
...
func (rb *Buffer) ReadFrom(r io.Reader) (n int64, err error) {
	...
	for {
		...
		if rb.w >= rb.r {
			...
			if m < 0 {
				panic("ring.Buffer.ReadFrom: reader returned negative count from Read")
			}
			rb.isEmpty = false
			rb.w = (rb.w + m) % rb.size
			n += int64(m)
...
func (rb *Buffer) WriteTo(w io.Writer) (int64, error) {
	...
	if rb.w > rb.r {
		...
		if m > n {
			panic("ring.Buffer.WriteTo: invalid Write count")
		}
		rb.r += m
		if rb.r == rb.w {
			rb.Reset()
...
func (rb *Buffer) grow(newCap int) {
	if n := rb.size; n == 0 {
		if newCap <= DefaultBufferSize {
			...
			newCap = mathutil.CeilToPowerOfTwo(newCap)
		}
	} else {
		doubleCap := n + n
		if newCap <= doubleCap {
